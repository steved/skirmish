...
float vector_euclidian_distance(gsl_vector *vector1, gsl_vector *vector2) {
  float xdiff = gsl_vector_get(vector1, 0) - gsl_vector_get(vector2, 0);
  float ydiff = gsl_vector_get(vector1, 1) - gsl_vector_get(vector2, 1);
...
// adapted bresenham's line drawing algorithm
bool hit_unit(bool occupied[][MAP_SIZE], int x, int y, int x2, int y2) {
  int dx = abs(x2 - x);
  int dy = abs(y2 - y);
